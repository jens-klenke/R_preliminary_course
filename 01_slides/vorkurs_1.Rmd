...
# download required icon libraries
...
R / `r r_black` ist eine freie Programmiersprache für statistische Berechnungen/Grafiken und wurde von Statistikern für Anwender mit statistischen Aufgaben entwickelt.
...
$\Rightarrow$ **Wir arbeiten daher mit RStudio** `r smiley`  
...
`r r_black` stellt besonders große bzw. besonders kleine Zahlen mit Hilfe der [wissensschaftlichen Notation](https://de.wikipedia.org/wiki/Wissenschaftliche_Notation) dar. Die Zahl -2.449294e-16 aus dem `r r_black`-Output ist
...
**Achtung:** `T` und `F` sollten nicht als Variablen verwendet werden (warum?). Die lange Version ist außerdem weniger fehleranfällig. 
...
Um eine Untermenge der Elemente eines Vektors anzusprechen (*Subsetting* genannt), nutzen wir in eckigen Klammern eingeschlossene *Indizes*.
...
**Zwei extrem wichtige Fähigkeiten beim Programmieren, die gleich schon mal geübt werden können!** `r grin_wink`
...
1. Erzeuge einen Vektor `numbers` mit den Elementen $$\{4 , 6, -3, 2.5, 18, \pi, 85\}$$

    *Hinweis:* Die Zahl $\pi$ ist in `r r_black` bereits als `pi` vordefiniert.

2. Berechne das arithmetische und das harmonische Mittel von `numbers`. 
...
3. Wir vermuten, dass die höchste und die niedrigste Zahl zu verzerrten Ergebnissen führen und entscheiden darum, diese Werte zu ignorieren. Ersetze beide Werte durch `NA` und berechne die Mittelwerte aus Aufgabe 2 erneut.
...
<ol start="4">
    <li>Nutze die Funktion <code>seq()</code> um die Folge \(\begin{pmatrix} 0, 0.5, 1, 1.5, \ldots, 99, 99.5, 100 \end{pmatrix}\) zu erzeugen. Wie viele Elemente besitzt dieser Vektor? Überprüfe Deine Vermutung mit <code>length()</code></li>
    <li>Erzeuge einen neuen Vektor <code>characters</code> mit den Elementen \(\begin{pmatrix} a & a & a  & b & b & b & b & c & c \\ \end{pmatrix}\). Finde heraus, wie die Funktion <code>rep()</code> funktioniert und nutze diese.</li>
    <li>Überschreibe nun den Vektor <code>characters</code> mit \(\begin{pmatrix} x & y & z & x & y & z & x & y & z \\ \end{pmatrix}\). Nutze wieder die Funktion <code>rep()</code>.</li>
    <li>Ersetze nun alle Einträge mit dem Inhalt \(z\) durch ein \(v\).</li>
</ol>
...
<ol start="8">
    <li> Übernehme folgenden Code und berechne \(a + b\),  \(a + x\),  \(a + y\) und \(a + z\). Finde heraus, wie `r r_black` jeweils vorgeht.</li>
</ol>
...
<ol start="9">
    <li>Erzeuge einen Vektor mit den Elementen \(\begin{pmatrix} 1 & 2 & 3 & a & b \\ \end{pmatrix}\) (d.h. eine Mischung aus <code>numeric</code> und <code>character</code>). Was passiert? Schreibe einen Kommentar.</li>
</ol>
...
1. Erzeuge mit dem Inputvektor `1:12` und <code>matrix()</code> folgende Matrix <code>X</code>. 
...
<ol start="2">
    <li>Nimm die Matrix aus 1. und vertausche die Spalten. Das Ergebnis soll an die Variable <code>Y</code> übergeben werden.</li>
    <li>Berechne \(XY^{T}\).</li>
</ol>
...
<ol start="4">
    <li>Erzeuge eine \(2 \times 2\) Matrix aus der 2. und 5. Zeile der Matrix <code>X</code>.</li>
    <li>Erzeuge die Matrix <code>X</code> mit <code>X <- matrix(8:-7, nrow = 4)</code>.</li>
    <ul>
    <li>Ersetze die Elemente auf der Hauptdiagonalen durch <code>NA</code>s.</li>
    <li>Ersetze jetzt alle <code>NA</code>s in der Matrix durch <code>1</code>. Nutze dazu die Funktion <code>is.na()</code>.</li>
    </ul>
</ol>
