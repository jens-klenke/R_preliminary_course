...
#### required packages ####
# packages
suppressWarnings(source(here::here('neu/00_packages/packages.R')))
```
...
```{r, fig_dens_2, echo = FALSE, out.width = '50%',  fig.align = "center"}
   knitr::include_graphics(here::here('neu/01_slides/Untitled.png'), auto_pdf = FALSE)
```
...
<ol start = "2">
    <li>Es sei <code>z <- c(1, 2, NA, 4)</code>. Überprüfen Sie die folgenden Aussagen mittels einer Logikabfrage in `R`.</li>
</ol>
...
<ol start = "4">
    <li>Verschaffen Sie sich einen Überblick über den Datensatz <code>mtcars</code> (dieser ist in base R bereits geladen)</li>
    <ul>
      <li>Aus wie vielen Variablen besteht er? Welche Klasse haben die einzelnen Variablen?</li>
    </ul>
...
Listen werden mit der Funktion `list()` erzeugt. Ein .blue[**Vorteil**] von Listen ist, dass die einzelnen Elemente von unterschiedlicher Größe und Typ sein können.
...
Solange die .blue[<code>condition</code>] wahr ist, wird der Schleifenkörper immer wieder ausgeführt. Vor dem ersten und nach jedem weiteren Durchlauf wird die .blue[<code>condition</code>] erneut evaluiert.
