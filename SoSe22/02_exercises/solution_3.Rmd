...
author2: 'Janine Langerbein und Marco Schwarzbach'
...
## Sei $X\sim t(5)$. Berechne $P(X<6)$, $P(3<X\leq7)$ und $P(X>4)$.
...
## Berechne das $0.95$-Quantil einer $F(4, 5)$-verteilten Zufallsvariable.

```{r}
    qf(0.95, df1 = 4, df2 = 5)
```

## Berechne die Wahrscheinlichkeit dafür, den Jackpot im Lotto zu gewinnen (d.h. 6 Richtige aus 49). Vernachlässige Zusatz- und Superzahlen. (Hinweis: Benutze die hypergeometrische Verteilung.)
...
## Erzeuge 20 $\chi^2(5)$-verteilte Zufallszahlen ohne (!) dabei die `rchisq()`-Funktion zu benutzen. 

*Hinweis*: $\chi^2(n)=\sum_{i=1}^n Z_i^2$ mit $Z_i\sim\mathcal{N}(0,1)$ für alle $i=1,...,n$.

```{r}
    # seed damit bei beiden Befehlen die Ergenisse gleich sind
    set.seed(1549) 
    # schnell: 
...
## Ziehe 10 mal $n = 10000$ standardnormalverteilte Zufallszahlen und berechne für jede Ziehung das arithmetische Mittel. Betrachte die 10 Mittelwerte. Was fällt auf?
...
**Zusatzaufgabe**: Wdh. mit Chauchy-Verteilung
...
## Zusatz: Prüfe, dass das Integral über die Dichtefunktion einer $\chi^2(15)$-verteilten Zufallsvariable 1 ist. 
...
## Kopiere obigen Code und nimm an, dass dieser eine IQ-Testreihe mit 1000 Probanden simuliert. Zeichne ein Histogram der Ergebnisse. Beschrifte den Plot und setze die Bereich von x- und y-Achse auf $[40,160]$ bzw. $[0,0.03]$.
...
## Zeichne die zugrundeliegende, theoretische Dichtefunktion, d.h. eine Normalverteilung mit $\mu=100$ und $\sigma=15$, in den plot.
...
## Zeichne ein blaues Dreieck an das Maximum der theoretischen Dichte.
...
## Markiere sowohl das 0.025- als auch das 0.975-Quantil der theoretischen Dichte mit vertikalen grünen Linien.
...
## Betrachte im Folgenden den Datensatz `faithful`, der Daten zum Old Faithful Geysir im Yellowstone Nationalpark enthält. Sowohl die Dauer einer Eruption in Min. (`eruptions`) als auch die Wartezeit bis zur nächsten Eruption in Min. (`waiting`) sind als Variablen im Datensatz verfügbar. Unterstelle das untenstehende Regressionsmodell und schätze die Parameter $\beta_0$, $\beta_1$. Zeichne anschließend eine geeignete Grafik und interpretiere.
...
## Erhalte mit `summary()` einen Überblick über die Regression aus 3.1. Interpretiere die geschätzten Koeffizienten und speichere anschließend das $R^2$ (*Multiple R-squared*) in `R2`.
...
## Amgemommen wir beobachten einen weiteren Datenpunkt für die Dauer einer Eruption von $X_{new}=4$. Triff eine Vorhersage für die Wartezeit bis zur nächsten Eruption!
